...
const eliminarProductos = () => {
    ...
    Inertia.post("/eliminar-posgrado", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
...
const registrarNuevo = () => {
    ...
    if (
        ...
        Inertia.post('/registro-posgrado', data, {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                productDialog.value = false
...
const onRowEditSave = (event) => {
    ...
    Inertia.post(`/editar-posgrado/${newData.id}`, data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            chartData.value = setChartData();
...
const setChartData = () => {
    ...
    const hombresTotal = props.registrosPosgrado.reduce((total, item) => total + item.hombres1 + item.hombres2, 0);
    const mujeresTotal = props.registrosPosgrado.reduce((total, item) => total + item.mujeres1 + item.mujeres2, 0);
        ...
        <div class="max-w-[99.9%] m-auto p-2">
            ...
            <div class="contenedorGraficaTabla">
                ...
                <div class="contendorTabla">
                    <DataTable :value="registrosPosgrado" ref="dt" class="p-datatable-sm" showGridlines stripedRows
