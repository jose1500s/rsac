...
const eliminarProductos = () => {
    ...
    Inertia.post("/eliminar-automotriz", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
...
const registrarNuevo = () => {
    ...
    if (
        ...
        Inertia.post('/registro-automotriz', data, {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                productDialog.value = false
...
const onRowEditSave = (event) => {
    ...
    Inertia.post(`/editar-automotriz/${newData.id}`, data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            chartData.value = setChartData();
...
const setChartData = () => {
    ...
    const hombresTotal = props.registrosAutomotriz.reduce((total, item) => total + item.hombres1 + item.hombres2, 0);
    const mujeresTotal = props.registrosAutomotriz.reduce((total, item) => total + item.mujeres1 + item.mujeres2, 0);
