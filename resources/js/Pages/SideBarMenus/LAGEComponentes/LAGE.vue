...
const eliminarProductos = () => {
    ...
    Inertia.post("/eliminar-lage", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
...
const registrarNuevo = () => {
    ...
    if (
        ...
        Inertia.post('/registro-lage', data, {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                productDialog.value = false
...
const onRowEditSave = (event) => {
    ...
    Inertia.post(`/editar-lage/${newData.id}`, data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            chartData.value = setChartData();
...
const setChartData = () => {
    ...
    const hombresTotal = props.registrosLage.reduce((total, item) => total + item.hombres1 + item.hombres2, 0);
    const mujeresTotal = props.registrosLage.reduce((total, item) => total + item.mujeres1 + item.mujeres2, 0);
